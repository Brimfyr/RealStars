...
internal static class StarShaders
{
    ...
    public static readonly (string Name, string Find, string Replace)[] Edits =
    {
        ...
        ("RayMarching/RayMarchingTest.glsl",
         ...
         + "\n"
         + "    // Past the limb, the white ring our sprite draws there: the disc as an eye sees it,\n"
         + "    // spread past its edge by the same point spread function, from the same peak - the\n"
         + "    // star shader's sum, written out, since this file has not got Tuning. Without it\n"
         + "    // the sphere was the bare disc and the sprite the disc and its ring, and across the\n"
         + "    // handover the Sun changed size. It is never brighter than the limb it comes off,\n"
         + "    // and it adds its light: the colour is divided by the cover the blend multiplies by.\n"
         + "    float sunMag = 4.83 + 5. * log(max(length(global.lighting.sunPosition.xyz), 1.)\n"
         + "                                   / 3.0856775814913673e17) * 0.4342944819;\n"
         + "    sunMag = sunMag < -6. ? -6. + (sunMag + 6.) * 0.35 : sunMag;\n"
         + "    float sunPeak = pow(10., -0.4 * (sunMag - 5.)) * 0.886 * 3.5 / 3.14159265;\n"
         + "    float sunLevel = " + SunLevelGlsl + ";\n"
         + "    float past = max(impact - sunRadius, 0.) / edge;             // pixels past the limb\n"
         + "    float ringShare = min(sunPeak * pow(1. + past * past, -4.5) / sunLevel, 0.4);\n"
         + "    float ringCover = min(ringShare * sunLevel, 1.);\n"
         + "    vec3 sunLight = global.lighting.sunColor.rgb;\n"
         + "    vec3 ringColour = sunLight / max(max(sunLight.r, sunLight.g), max(sunLight.b, 1e-6))\n"
         + "                    * ringShare / max(ringCover, 1e-6);\n"
         + "    total_color = mix(ringColour, total_color, alpha);\n"
         + "    alpha = max(alpha, ringCover);\n"
         + "\n"
